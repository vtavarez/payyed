    ...
    useEffect(() => {
        ...
        try {
            const fetchData = async () => {
                const {
                    data: { rates },
                } = await axios.get(`https://openexchangerates.org/api/latest.json`, params);
            ...
            <Formik
                ...
                onSubmit={({ email, senderAmount }, { setSubmitting }) => {
                    ...
                    dispatch({
                        ...
                        payload: {
                            email,
                            senderAmount: parseAmount(senderAmount).toFixed(2),
                            fee: getFee(parseAmount(senderAmount)).toFixed(2),
                            senderCurrency: senderCurrency.toUpperCase(),
                            total: (
                                parseAmount(senderAmount) + getFee(parseAmount(senderAmount))
                            ).toFixed(2),
                        },
                    });
                }}
                ...
                {({ handleBlur, handleChange, handleSubmit, values }) => (
                    <Form onSubmit={handleSubmit}>
                        ...
                        <ExchangeRate>
                            ...
                            <Rate>
                                1 {senderCurrency.toUpperCase()} = {exchangeRate(recipientCurrency)}{" "}
